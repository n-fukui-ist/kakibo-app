...
try:
    ...
    if not df.empty:
        ...
        # --- 計算エリア ---
        # 1. 現在の残高
        current_balance = df["金額"].sum()

        # 2. 未回収の立替金（これから戻る予定）
        # 「立替」の合計（マイナスなので絶対値にする）
        tatekae_sum = abs(df[df["カテゴリ"] == "立替"]["金額"].sum())
        # 「立替回収」の合計
        kaishu_sum = df[df["カテゴリ"] == "立替回収"]["金額"].sum()
        
        # 未回収 = 立替した総額 - 回収した総額
        pending_amount = tatekae_sum - kaishu_sum

        # 3. 見込み残高（現在の残高 + 未回収分）
        future_balance = current_balance + pending_amount
        ...
        col1.metric("現在の残高", f"¥{current_balance:,}")
